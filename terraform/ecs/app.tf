...
data "template_file" "app-task" {
  template = file("templates/app.json.tpl")
  vars = {
    REPOSITORY_URL = replace(aws_ecr_repository.develop.repository_url, "https://", "")
  }
}

resource "aws_ecs_task_definition" "app-task-definition" {
  family                = "myapp"
  container_definitions = data.template_file.app-task.rendered
}
